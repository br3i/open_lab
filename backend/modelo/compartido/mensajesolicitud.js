...
const emailService = require('./enviarcorreo'); // Servicio de correo

module.exports.mensajeSolicitud = async function (objSolicitud) {
    try {
        // Validar que los correos existan y sean un array no vacío
        ...
        const mensaje = objSolicitud.cuerpo; // Contenido del mensaje
        const correos = objSolicitud.correos; // Array de correos

        // Generar el cuerpo en formato base64
        const plantillaHTML = `
<html>
<head>
    <style>
...
`;
        ...
        // Configurar el objeto que será enviado al método EnviarCorreo
        const correoData = {
            strAsunto: "Solicitud Empresa",
            strBody: strBody,
            lstArchivosAdjuntos: [], // Si no hay adjuntos, dejamos el array vacío
            lstReceptores: correos.map(email => ({ email: email })) // Generar formato requerido con "email"
        };
