...
const tools = require('../tools'); // Asumiendo que tools contiene la función Generarpdfhtml
            ...
            return `
              <div class="clause">
                ...
                ${clausula.subclausulas.map(subclausula => {
                  // Verificar si algún item contiene <li>
...
async function pdfContratoGenerar(objPrevisualizacion) {
  try {
    ...
    const datosCoordinadoBar = await modelodatosorganizacion.BuscarPersonalIdPersonal(
      Number(datos?.objConvenioEmpresa?.ouconvenio_idcoordinadorbar)
    );
    const datosOrganizacion = await modelodatosorganizacion.ListadoDatosOrganizacionActivos();
    const organizacion = datosOrganizacion.data[0];
